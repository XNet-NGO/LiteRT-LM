...
CachedSession::CachedSession(
    std::unique_ptr<SessionInterface> session, Tokenizer* tokenizer,
    std::optional<VisionExecutorProperties> vision_properties,
    std::optional<AudioExecutorProperties> audio_properties)
    : session_(std::move(session)),
      tokenizer_(tokenizer),
      vision_properties_(vision_properties),
      audio_properties_(audio_properties) {}
...
CachedSession::RunPrefillAsync(
  ...
  // Preprocess the input contents upfront. This ensures all text inputs are
  // converted to TensorBuffers of token IDs.
  ...
  if (match_result.matched_elements == 0) {
    ...
    return session_->RunPrefillAsync(preprocessed_contents,
  ...
  return session_->RunPrefillAsync(sliced_contents,
                                   std::move(wrapped_callback));
}
